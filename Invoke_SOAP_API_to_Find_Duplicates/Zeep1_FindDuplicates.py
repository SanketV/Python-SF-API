...
print("Call login()")
response = client.service.login(username='sanket.vaidya.fsc@jackson.com.qa',password='Jackson3!NhCgdAONzaxx0IiknUHKAQbx')
...
#print("Duplicate Contact name : " + result.body.result[0].duplicateResults[0].matchResults[0].matchRecords[0].record._value_1[2].text)

inputdata = pd.read_csv(".\\data\\Contacts1.csv", encoding='unicode_escape')
#pp.pprint(inputdata)
...
def make_csv(contactList):
    with open('data\Out_ContactsWithDuplicates.csv','w', newline='') as csvfile:
